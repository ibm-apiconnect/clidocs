...
Return the data needed to populate the Inactive consumer organizations report table at space scope.

Asynchronous report generation:
This is an asynchronous endpoint. The typical workflow is:

1. Initial request: Make your first API call with the desired parameters (type, timeframe, start/end dates, etc.)
...
- Reports are cached based on the scope, type, and time period parameters

```
apic reports:spaceInactiveCorgReportTable --mode analytics [flags]
```
...
```
  ...
  -c, --catalog string                                  Catalog name or ID. (required)
      ...
      --inactive-consumer-report-type no_applications   Type of inactive consumer report to query. Specifies which category of inactive consumer organizations to retrieve. Possible values: no_applications (Consumer organizations with no applications created), `no_subscriptions` (Consumer organizations with no active subscriptions to products), `no_traffic` (Consumer organizations with no API traffic at all), `no_recent_traffic` (Consumer organizations with no recent API traffic within the specified time period). (required)
