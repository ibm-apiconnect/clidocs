...
Return the data needed to populate the Inactive product report table at catalog scope

```
apic reports:catalogInactiveProductReport --mode analytics [flags]
```
...
```
  ...
  -c, --catalog string                                  Catalog name or ID. (required)
      ...
      --inactive-product-report-type no_subscriptions   Type of inactive product report to query. Specifies which category of inactive products to retrieve. Possible values: no_subscriptions (Products with no active subscriptions), `no_traffic` (Products with no API traffic at all), `no_recent_traffic` (Products with no recent API traffic within the specified time period). (required)
